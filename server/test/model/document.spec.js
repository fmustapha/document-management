        ...
        db.Document.create(emptyString)
          ...
          .catch((error) => {
            if (field === 'access') {
              ...
              .to.equal('invalid input value for enum "enum_Documents_access": " "');
            } else {
              expect(error.message)
              .to.equal('Validation error: This field cannot be empty');
            }
          });
        done();
      });
  ...
  describe('ACCESS Violation', () => {
    ...
    (done) => {
      ...
      db.Document.create(accessError)
        ...
        .catch((error) => {
          expect(error.message)
            .to.equal('invalid input value for enum "enum_Documents_access": "andela"');
        });
      done();
    });
  });
